# Project code from Chapter 2 of Python Playground 
# by Mahesh Venkitachalam
# slight updates to code by Rachel Hunt
...
def saveDrawing():
    ...
    fileName = '/home/chouchou/PyPlate/cogs/spiro-' + dateStr
    print('saving drawing to %s.eps/png' % fileName)
    # get the tkinter canvas - turtle has a getscreen() that
    # 'returns the TurtleScreen object the turtle is drawing on
    
    # canvas = turtle.getcanvas()
    # the above was throwing an error later at 187 because canvas 
    # did not have the postscript attribute

    # the below saves the tkinter screen and then the screen can call
    # the getcanvas function
    # covered here: https://stackoverflow.com/questions/53266892/
    # how-to-turn-python-turtle-graphic-output-into-an-image-file?rq=1
    ts = turtle.getscreen()
    ts.getcanvas().postscript(file=fileName+'.eps') 
    
    # save the drawing as a postcript image
    # canvas.postcript(file = (fileName + '.eps'))
    
    # use the Pillow module to convert the postscript image file to PNG
    # img = Image.open(fileName + '.eps')
    # img.save(fileName + '.png', 'png')
    
    # changed to with statement below to ensure file close is 
    # covered in the event of an accident
    with Image.open(fileName + '.eps') as img:
        img.save(fileName + '.png', 'png')
